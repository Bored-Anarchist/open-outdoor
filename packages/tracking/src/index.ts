...
export interface EnergyBudget {
  ...
  readonly maximumBatteryPercentPerHour: number | null;
  readonly maximumResidentMemoryP95MiB: number;
}

export interface EnergyRunEvidence {
  ...
  readonly residentMemoryP95MiB: number;
  readonly thermalWarning: boolean;
  readonly continuousRetry: boolean;
  readonly unintendedBackgroundSession: boolean;
  readonly missedStop: boolean;
}

export interface EnergyEvaluation {
  readonly status: 'passed' | 'blocked';
  readonly measuredRuns: number;
  readonly batteryPercentPerHour: readonly number[];
  readonly reasons: readonly string[];
}

export function evaluateEnergyEvidence(
  evidence: readonly EnergyRunEvidence[],
  budget: EnergyBudget,
): EnergyEvaluation {
  ...
  if (budget.maximumBatteryPercentPerHour === null) {
    reasons.push(`${budget.mode} requires an approved battery budget`);
  }
  ...
  const rates = runs.map((run) => {
    ...
    if (run.residentMemoryP95MiB > budget.maximumResidentMemoryP95MiB) {
      reasons.push(`run ${run.runId} exceeds the resident-memory budget`);
    }
    if (
      run.thermalWarning ||
      run.continuousRetry ||
      run.unintendedBackgroundSession ||
      run.missedStop
    ) {
