...
6. Enable VoiceOver, the largest Dynamic Type size, Bold Text, Increase Contrast, Differentiate Without Color, Reduce Motion, and dark mode. Use the runner's one-at-a-time Start, Pause, Resume, and Finish controls, keep VoiceOver enabled, and record the single usability result.
7. Export the consolidated report.

The runner automatically captures process relaunch, durable tracker recovery, authorization transitions, safe sensor stop, screen-off duration, network transitions, weak GPS, explicit stop, resident-memory samples, elevation error, accessibility settings/control order, the iPhone hardware identifier, the embedded source commit, app/build metadata, and timestamps. The production tracker persists throttled relative-altitude observations during vertical movement even when GPS does not emit a new distance-filtered point.

## Targeted retries

After a completed run, a failed elevation check can be repeated with a short climb-only recording while preserving passed tracker and memory evidence. Once elevation passes, accessibility can be retried independently. The runner persists the ordered accessibility-control progress and will not accept **Accessibility flow is usable** unless every required setting is detected while VoiceOver remains on.

Targeted retries are valid only on the same candidate commit. Installing a changed candidate requires a fresh guided run; the report's embedded source commit and the ingester's current commit must match. This prevents a final report from silently combining measurements from different binaries.
...
- Accessibility: VoiceOver and the complete worst-case display configuration must be detected, Start/Pause/Resume/Finish must be exercised in order, and one human usability confirmation is required.
- Device/build identity: hardware identifier iPhone14,7, iOS 26.6, the local bundle ID (including a sanctioned ten-character AltServer team suffix), and the exact embedded source commit must match.
