...
WP-009 published the binding non-energy release-config values, current iPhone 14/iOS 26.6 profile, measurement procedure, and risk disposition in the Phase 0 gate record and report. The historical iOS 26.2 launch evidence remains recorded. The review result remains `BLOCKED` by WP-007's protection/pre-first-unlock case and the 30-minute memory budget. `BUD-REC-001` passed with Start p95 13 ms and Stop p95 2 ms against the 500 ms threshold. `BUD-REC-002` passed with sequence 2 preserved across forced termination and relaunch, a zero observed committed-data gap, and monotonic continuation through sequence 5. WP-008 is owner-accepted on existing evidence with its unexecuted physical cases retained as explicit residual risk. The bounded hosted-CI window passed 20/20 applicable runs with zero avoidable failures on 2026-08-23. Measured energy acceptance is deferred and is not a Phase 0/Phase 1 blocker. Phase 1 must not be scheduled until a later gate record passes every current blocking item.
