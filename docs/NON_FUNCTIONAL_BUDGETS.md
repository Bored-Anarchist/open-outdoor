...
WP-009 published the binding non-energy release-config values, iPhone 14/iOS 26.2 profile, measurement procedure, and risk disposition in the Phase 0 gate record and report. The review result remains `BLOCKED`: WP-007/WP-008 physical runs and protection/system-backup inspection remain incomplete. The bounded hosted-CI window passed 20/20 applicable runs with zero avoidable failures on 2026-08-23. Measured energy acceptance is deferred and is not a Phase 0/Phase 1 blocker. Phase 1 must not be scheduled until a later gate record passes every current blocking item.
